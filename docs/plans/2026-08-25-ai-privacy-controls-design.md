...
**Correction found while planning implementation:** the design originally
also named a "voice-mode badge" in `src/app/(app)/write/page.tsx`. It
doesn't exist — `VoiceRecorder.tsx` renders no "End-to-End Encrypted"
claim at all during recording/transcription (the moment audio genuinely
does leave the device raw, for Whisper), and both badge occurrences that
actually exist in `write/page.tsx` (the composing-stage footer and the
post-save screen) are generic to every save regardless of whether the
text originated by voice or typing — and accurate on both, since the
final entry text really is encrypted before storage either way. Nothing
to fix there.
...
- `src/app/(app)/playback/story/page.tsx:96` and `:434` — replace with
  copy that's specific to what actually happened, e.g. "Your entries are
  encrypted · this story was generated by AI from your decrypted text,"
  or similar, finalized during implementation to match the page's
  existing tone.

The other ~23 occurrences (including both in `write/page.tsx`) are
accurate as-is and are explicitly left untouched.
