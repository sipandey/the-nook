...
**A client-side off switch covers both §6.4 and §6.5** — plus manifestation-signal detection, the third and only other plaintext-to-OpenAI path (`POST /api/manifestations` → `detectManifestationSignals`, gated the same way; not diagrammed separately here since it's structurally identical to §6.4's "decrypt locally, POST plaintext, never persisted" shape, just triggered after a save rather than from a "Play" tap). `src/lib/hooks/useAiEnabled.ts` reads/writes a boolean via Clerk `unsafeMetadata` (mirroring `tone`'s existing storage — see §10.2). Every one of the three call sites (`usePlaybackNarrative`'s trigger in `playback/story/page.tsx`, the voice-mode render in `write/page.tsx`, and `useSignalDetector.ts`) checks it and returns *before* the `fetch`, not after. This has to be client-side: a server-side check on `/api/ai/*` can't undo a request that already carried plaintext over the wire to reach it — by the time the route handler runs, the exposure the switch exists to prevent has already happened. Daily prompts (§10.2) aren't gated by this switch at all — they send no entry content in the first place (`generateDailyPrompt(tone, [])`, an empty summaries list), so there's nothing for the switch to prevent. Default is on for existing users, disclosed rather than silently flipped off — see `docs/plans/2026-08-25-ai-privacy-controls-design.md`.
...
- Resolved since the last pass:
  ...
  - **PWA installability.** Service worker registered — see `src/app/sw.ts`, `src/app/serwist/[path]/route.ts`, `SerwistProvider` in `src/app/providers.tsx`. Offline app-shell caching confirmed working (verified live: killed the local server process entirely, a previously-visited page stayed fully available, a never-visited one fell back to the on-brand `/~offline` page). `manifest.json` and icon sizes already existed — Add to Home Screen is now confirmed on a real iOS device (the user installed it and used it in standalone mode); Android remains untested.
  - **Production error monitoring (NK-06).** Built via `@sentry/nextjs`, active only in real production (`src/lib/monitoring/sentryEnabled.ts`, matching `preview.ts`'s existing `VERCEL_ENV` gate). Every data-collection category Sentry could otherwise send by default — most importantly `stackFrameVariables`, which defaults to sending local variable *values* from a crashing function's stack frame regardless of `sendDefaultPii` — is explicitly turned off in `src/lib/monitoring/sentryOptions.ts`. See `docs/plans/2026-08-25-sentry-error-monitoring-design.md` for the full reasoning, including a mid-implementation correction (the design's first draft trusted `sendDefaultPii: false` alone; checking the installed SDK's actual type definitions directly showed that was wrong).
